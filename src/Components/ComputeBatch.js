...
class ComputeBatch extends React.Component {


	constructor(props) {
		super(props);
		this.state = { vm_name: '', vm_ip: '', vm_template: '', view: 'terraform', vmCreationDetail: '', rand: '', vm_iso_image: '', guestos: '', vm_cpus: '', loading: false, number_of_VMs: '' };
	...
	handleSubmit = (event) => {
		...
		var url =`http://${process.env.REACT_APP_Terraformizer_Host}/v1/terraform-vsphere-createbatchvms-template/my_workspace` + this.state.rand;
		//alert(url);
		this.state.vm_name = this.state.vm_name + "-" + this.state.rand;
		var createVMPayload = JSON.stringify({ "vm_name": this.state.vm_name, "vm_template_name": this.state.vm_template, "vm_ip_prefix": this.state.vm_ip, "number_of_VMs": Number(this.state.number_of_VMs) });
		if (this.state.vmcreationoption == 'isoimage') {
			createVMPayload = JSON.stringify({ "vm_name": this.state.vm_name, "vm_image": this.state.vm_iso_image, "vm_cpus": Number(this.state.vm_cpus), "guest_id": this.state.guestos, "number_of_VMs": Number(this.state.number_of_VMs)  });
			url = `http://${process.env.REACT_APP_Terraformizer_Host}/v1/terraform-vsphere-batchvmscreation/my_workspace` + this.state.rand;
		}
	...
	handleClick = (event) => {
		...
		this.state.rand = this.state.vm_name.substring(this.state.vm_name.lastIndexOf('-', this.state.vm_name.lastIndexOf('-')-1) + 1, this.state.vm_name.lastIndexOf('-'));
		//alert(this.state.rand);
		var url = `http://${process.env.REACT_APP_Terraformizer_Host}/v1/terraform-vsphere-createbatchvms-template/my_workspace` + this.state.rand;
		//alert('submitting to url: '+url);
		var deleteVMPayload = JSON.stringify({ "vm_name": this.state.vm_name, "vm_template_name": this.state.vm_template, "vm_ip_prefix": this.state.vm_ip, "number_of_VMs": Number(this.state.number_of_VMs)  });
		//var modifiedDeleteVMPayload = deleteVMPayload.replace(/"/gi, '\\"');
		if (this.state.vmcreationoption == 'isoimage') {
			deleteVMPayload = JSON.stringify({ "vm_name": this.state.vm_name, "vm_image": this.state.vm_iso_image, "vm_cpus": Number(this.state.vm_cpus), "guest_id": this.state.guestos,"number_of_VMs": Number(this.state.number_of_VMs)  });
			url = `http://${process.env.REACT_APP_Terraformizer_Host}/v1/terraform-vsphere-batchvmscreation/my_workspace` + this.state.rand;
		}
	...
	myChangeHandler7 = (event) => {
		this.setState({ number_of_VMs: event.target.value });
	}
				...
				<form onSubmit={this.handleSubmit} style={{ cursor: 'hand' }}>
					<table>
						...
						<tr>
							...
							<th> Enter the VM IP Address Prefix to be Assigned: </th>
							<td>
						...
						<tr>
							<th>Enter the number of VM's to be created</th>
							<td><input name="number_of_VMs" id="number_of_VMs" type="text" onChange={this.myChangeHandler7}></input></td>
						</tr>
					</table>
